/**
 * Environment Configuration for Staff Dashboard
 * 
...
function getApiBaseUrl() {
    ...
    if (manualOverride) {
        console.log('🔧 [StaffConfig] Using manual API URL override:', manualOverride);
        return manualOverride;
    }
    
    const env = getEnvironment();
    console.log('🌍 [StaffConfig] Detected environment:', env);
    
    switch (env) {
        case 'development':
            console.log('🔗 [StaffConfig] Using development API URL:', DEV_API_URL);
            return DEV_API_URL;
        case 'staging':
            console.log('🔗 [StaffConfig] Using staging API URL:', STAGING_API_URL);
            return STAGING_API_URL;
        case 'production':
            console.log('🔗 [StaffConfig] Using production API URL:', PROD_API_URL);
            return PROD_API_URL;
        default:
            console.log('🔗 [StaffConfig] Using default development API URL:', DEV_API_URL);
            return DEV_API_URL;
    }
}
...
const config = {
    ...
    setApiUrl: (newUrl) => {
        ...
        console.log('🔄 [StaffConfig] API URL manually updated to:', newUrl);
    }
};

// Log configuration on load
console.log('⚙️ [StaffConfig] Configuration loaded:', {
...
if (typeof window !== 'undefined') {
    window.STAFF_API_CONFIG = config;
    
    // Add helper function for easy URL changes during testing
    window.setStaffApiUrl = (url) => {
        config.setApiUrl(url);
        console.log('🔧 [StaffConfig] Use window.setStaffApiUrl("http://localhost:8080") to change API URL for testing');
    };
}
