...
const setupIntegration = async (req, res) => {
    try {
        ...
        // Determine user type - admin should keep admin role, not be converted to staff
        const userType = role;
...
const getIntegrations = async (req, res) => {
    try {
        ...
        console.log(`🔍 [WhatsAppController] Calling unifiedWhatsAppService.getUserIntegrations(${userId}, ${userType})`);
...
const initializeBaileysSession = async (req, res) => {
    try {
        const { _id: userId, role } = req.user;
        const userType = role;
        console.log(`🚀 [WhatsAppController] Initializing Baileys session for user ${userId} with userType ${userType}`);
        
        // Check global settings
        const settings = await globalSettingsService.getSettings();
        if (!settings.isWhatsAppServiceEnabled('baileys')) {
            return res.status(403).json({
                success: false,
                message: 'Baileys integration is disabled in global settings'
            });
        }

        // Initialize the session and get QR code in one call
        const result = await baileysWhatsAppService.initializeSession(userId, userType);

        if (result.success) {
            console.log(`✅ [WhatsAppController] Baileys session initialized successfully for user ${userId}`);
            ...
            res.status(400).json({
                success: false,
                message: 'Failed to initialize Baileys session',
                data: result
            });
        }

    } catch (error) {
        console.error('❌ [WhatsAppController] Error initializing Baileys session:', error.message);
        res.status(500).json({
            success: false,
...
const getBaileysSessionStatus = async (req, res) => {
    try {
        ...
        const status = baileysWhatsAppService.getSessionStatus(userId, userType);
...
const disconnectBaileysSession = async (req, res) => {
    try {
        ...
        await baileysWhatsAppService.disconnectSession(userId, userType);

        res.status(200).json({
            success: true,
            message: 'Baileys session disconnected successfully'
        });
...
const deleteIntegration = async (req, res) => {
    try {
        ...
        const result = await unifiedWhatsAppService.deleteIntegration(userId, userType);
        ...
        console.error('[WhatsAppController] Error deleting integration:', error);
        res.status(500).json({
            success: false,
 ...
 */
const sendBaileysMessage = async (req, res) => {
    try {
        const { _id: userId, role } = req.user;
        const userType = role;
        ...
        if (message.length > generalConfig.maxMessageLength) {
            return res.status(400).json({
                success: false,
                message: `Message too long. Maximum length is ${generalConfig.maxMessageLength} characters`
            });
        }

        const result = await baileysWhatsAppService.sendMessage(userId, userType, to, message, options);
        ...
        console.error('[WhatsAppController] Error sending Baileys message:', error);
        res.status(500).json({
            success: false,
            message: 'Error sending message',
            error: error.message
